...
public:
  Circularfade(std::shared_ptr<Base> mother, int id, int type, int start, int length, int mode, cv::Point mid, float radius, bool fade_direction, int offset, int stride);
  ...
  void manipulate(int start, int length, int mode, cv::Point mid, float radius, bool fade_direction, int offset, int stride);
